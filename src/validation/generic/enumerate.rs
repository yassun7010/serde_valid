...
pub fn validate_generic_enumerate<T, U>(value: &T, enumerate: &[U]) -> bool
where
...
mod tests {
    ...
    fn test_validate_integer_vec_type_is_true() {
        assert!(validate_generic_enumerate(&1, &[1, 2, 3]));
    }

    #[test]
    fn test_validate_integer_vec_type_is_false() {
        assert!(!validate_generic_enumerate(&1, &[2, 3, 4]));
    }

    #[test]
    fn test_validate_float_type_is_true() {
        assert!(validate_generic_enumerate(&0.9, &[0.9, 2.3, -3.0]));
    }

    #[test]
    fn test_validate_float_type_is_false() {
        assert!(!validate_generic_enumerate(&0.9, &[0.8, 2.3, -3.0]));
    }

    #[test]
    fn test_validate_str_type() {
        assert!(validate_generic_enumerate(&'a', &['a', 'b', 'c']));
    }
    ...
    fn test_validate_vec_type() {
        assert!(validate_generic_enumerate(
            &vec!["a"],
            &[vec!["a"], vec!["b"], vec!["c"]]
        ));
    }

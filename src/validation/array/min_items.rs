...
mod tests {
    ...
    fn test_validate_array_min_items_is_true() {
        assert!(ValidateMinItems::validate(&[1, 2, 3], 3).is_ok());
    }

    #[test]
    fn test_validate_array_min_items_is_false() {
        assert!(ValidateMinItems::validate(&[1, 2, 3], 4).is_err());
    }

    #[test]
    fn test_validate_array_min_items_vec_is_true() {
        assert!(ValidateMinItems::validate(&vec!['a', 'b', 'c'], 3).is_ok());
    }

    #[test]
    fn test_validate_array_min_items_array_is_true() {
        assert!(ValidateMinItems::validate(&['a', 'b', 'c'], 3).is_ok());
    }
}

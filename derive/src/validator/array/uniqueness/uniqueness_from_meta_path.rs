...
pub fn extract_array_uniqueness_validator_from_meta_path(field: &NamedField) -> Validator {
    if let Some(option_field) = field.option_field() {
        Validator::Option(Box::new(extract_array_uniqueness_validator_from_meta_path(
            &option_field,
        )))
    } else {
        let message =

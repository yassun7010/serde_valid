...
use crate::validator::numeric::extract_numeric_multiples_validator_from_meta_name_value;
...
pub fn extract_validator_from_nested_meta_name_value(
    ...
    match path_ident.to_string().as_ref() {
        "multiple_of" => {
            return Some(extract_numeric_multiples_validator_from_meta_name_value(
                field, lit,
            ))
        }
        "pattern" => {

...
pub fn extract_number_multiples_validator(field: &NamedField, lit: &syn::Lit) -> Validator {
    if let Some(array_field) = field.array_field() {
        Validator::Array(Box::new(extract_number_multiples_validator(
            &array_field,
            lit,
        )))
    } else if let Some(option_field) = field.option_field() {
        Validator::Option(Box::new(extract_number_multiples_validator(
            &option_field,
            lit,
        )))
    } else {
        Validator::Normal(inner_extract_number_multiples_validator(field.ident(), lit))
    }
}

fn inner_extract_number_multiples_validator(
